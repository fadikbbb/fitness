...
exports.updatePasswordValidationMiddleware = [
  body("newPassword")
    .isLength({ min: 8 })
...
exports.userValidationMiddleware = [
    body("email")
        .isEmail()
    ...
    body('dateOfBirth')
        .custom(value => {
            if (value) {
                ...
                const eighteenYearsAgo = new Date(currentDate.setFullYear(currentDate.getFullYear() - 5));
                if (date > currentDate || date < minDate || date > eighteenYearsAgo) {
                    throw new Error('Date of birth must be in the past and at least 5 years ago');
                }
            }
