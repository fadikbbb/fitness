...
exports.createWorkoutPlan = async (body, userId) => {
  try {
    ...
    await Promise.all(
      body.exercises.map(async (exercise) => {
        if (
          !exercise.exerciseId ||
          !exercise.sets ||
          !exercise.minReps ||
    ...
    if (workoutPlan) {
      ...
      if (existingDay) {
        ...
        const newExercises = body.exercises.filter((exercise) => {
          return !existingDay.exercises.some(
            (existingExercise) =>
              existingExercise.exerciseId.toString() === exercise.exerciseId
          );
        });
        ...
        if (newExercises.length === 0) {
          throw new apiError(
            "All exercises are already added for this day",
            400
          );
        }
...
exports.updateExercise = async (
  ...
  try {
    ...
    const exercise = dayPlan.exercises.find((e) =>
...
exports.removeExercise = async (userId, planId, exerciseId, day) => {
  try {
    ...
    const dayContainingExercise = workoutPlan.days.find(
      (dayItem) =>
        dayItem.day === day &&
        dayItem.exercises.some((exercise) =>
    ...
    if (!dayContainingExercise) {
      throw new apiError(
        "Exercise not found in the specified day of the workout plan",
        404
      );
    }
    ...
    const dayAfterUpdate = updatedWorkoutPlan.days.find(
    ...
    if (dayAfterUpdate && dayAfterUpdate.exercises.length === 0) {
      ...
      await WorkoutPlan.updateOne({ userId }, { $pull: { days: { day } } });
    }
