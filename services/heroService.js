...
exports.updateHero = async (heroData, fileImage, logoImage, fileVideo) => {
  try {
    ...
    if (!hero) {
      if (fileImage) {
        fileImage = await uploadToStorage(
      ...
      if (fileVideo) {
        fileVideo = await uploadToStorage(
          fileImage.originalname,
          fileImage.mimetype,
          fileImage.buffer,
          "img"
      ...
      if (logoImage) {
        logoImage = await uploadToStorage(
      ...
      if (fileImage) {
        hero.heroImage = await updateFile(
          hero.heroImage,
          fileImage.originalname,
          fileImage.mimetype,
          fileImage.buffer,
      ...
      if (fileVideo) {
        hero.heroVideo = await updateFile(
          hero.heroVideo,
